...
for index, row in df.iterrows():
    ...
    if check_login_id_exists(postgres_conn, login):
        ...
        # query = "SELECT user_name, pswd, CASE WHEN is_active = true THEN 'Yes' ELSE 'No' END AS active_yn FROM ysr.ysr_branch_mst WHERE bank_id = %s"
        # cursor.execute(query, (login_id,))
        # existing_branch_details = cursor.fetchone()

        ## Convert is_active values from string to boolean
       ## existing_login_is_active = existing_login[1]
